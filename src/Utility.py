...
def pitch2MIDIPitch(pitch):
    interval2stepdiff = {0: 11, 1: 0, 1.5: 1, 2: 2, 2.5: 3,
                         3: 4, 4: 5, 4.5: 6, 5: 7, 5.5: 8, 6: 9, 6.5: 10, 7: 11}

    MIDIPitch = int(((pitch-1)//7+3)*12 + interval2stepdiff[pitch % 7])
    if 0 < MIDIPitch < 108:
        return MIDIPitch
    else:
        raise ValueError("MIDIPitch out of range")
...
def isValidPitch(pitch):
    if pitch <= 0:
        ...
        return False
    elif pitch % 7 not in {1, 1.5, 2, 2.5, 3, 4, 4.5, 5, 5.5, 6, 6.5, 0}:
        return False
    return True
...
def recodePitch(n):
    ...
    """
    stepDiff2Interval = {0: 1, 1: 1.5, 2: 2, 3: 2.5, 4: 3,
                         5: 4, 6: 4.5, 7: 5, 8: 5.5, 9: 6, 10: 6.5, 11: 7}
