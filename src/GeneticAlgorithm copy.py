    ...
    for main_parent in parents:
        for sub_parent in parents:
            ...
            for signature in main_parent.signature:
                invalid_elements.append(signature[1]-1)
            relative_elements = []
            for signatures in main_parent.allSignature:
                elements = []
                for element in signatures:
                    elements.append(element[1]-1)
                relative_elements.append(elements)
            print("all signature: \n", main_parent.allSignature)
            while 1:
                point = []
                index = random.randint(0, len(main_parent.cuttingPoint)-1)
                cuttingPoint = main_parent.cuttingPoint
                if cuttingPoint[index] not in invalid_elements:
                    for elements in relative_elements:
                        if cuttingPoint[index] in elements:
                            for end in elements:
                            ...
                            break
                    if point == []:
                        end = cuttingPoint[index]
                        start = 0 if index == 0 else cuttingPoint[index-1]+1
                        point.append((start, end))
                    break
